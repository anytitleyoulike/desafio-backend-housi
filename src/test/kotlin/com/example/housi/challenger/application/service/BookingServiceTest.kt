...
class BookingServiceTest {
    ...
    fun `Should get blocked dates with date provided`(){
        ...
        every { bookingRepositoryPort.findBySearchPeriod(id, searchPeriodDTO.start, searchPeriodDTO.end)} returns listOf(booking)
                listOf(booking)
    ...
    fun `Should get blocked dates without date provided`(){
        every { bookingRepositoryPort.findByPropertyId(id) } returns listOf(booking)
    ...
    fun `Should get booking by id with success`() {
        val listBooking = listOf(booking)
